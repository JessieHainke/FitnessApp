      ...
      <div className="flex pt-5 justify-center px-5">
        ...
        {program.workouts.map((workout, index) => (
          <div>
            ...
            <NavLink to={`/program/${id}`}><img src="../img/arrowToProgram.svg" className="top-6 right-6 fixed"></img></NavLink>
              <div className="absolute inset-y-1/2 w-64 inset-x-1/4">
                  <h1 className="text-2xl">{workouts.index}</h1>
                  <p>{program.workouts.duration} Min. - {workouts.category}</p>
              </div>
          </div>))}
      </div>
      <NavLink to={`/default-workout/${id}`} className="rounded-full bg-gradient-to-br from-orange to-pink px-12 py-2 flex justify-center mx-36 text-black">los!</NavLink>
    </div>    
  )
}
/*
